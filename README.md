...
| **API 응답 시간** | <100ms | ✅ 최적화 완료 |

**TOP 1 전략**: Next Candle Predictor (76.0점, B등급, 2,200 좋아요)
...
**❌ 절대 금지**:
- 원격서버에 직접 SSH 접속하여 코드 수정
- Docker 컨테이너 내부에서 파일 수정
- 로컬과 원격서버의 코드가 다른 상태로 방치
...
# 3. Git 커밋
git add .
git commit -m "feat: [수정 내용 명확히 기술]"

# 4. GitHub에 푸시
git push origin main

# 5. GitHub Actions 자동 배포 확인 (5-10분 소요)
gh run watch
